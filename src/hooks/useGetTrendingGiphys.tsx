...
// import { fetchTendringResponse } from "/src/axios";
...
//     const axiosResponse = await fetchTendringResponse();
//     const data = axiosResponse.data.data.map((gifs) => gifs);
...
import { fetchTendringGiphysResponse } from '../api/giphyApi'

const getTrendingGiphys = async () => {
    const { data } = await fetchTendringGiphysResponse()
    return data;
}

export const useGetTrendingGiphys = () => {
    useQueryClient();
    const query = useQuery({ queryKey: ['giphys'], queryFn: () => getTrendingGiphys()})
    return { query }
}
