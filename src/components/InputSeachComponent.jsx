...
export const InputSeachComponent = () => {
    ...
    const { startOpenAutoComplete, startCloseAutoComplete, isAutoCompleteOpen } = useSetUi();
    ...
    const onInputChange = ({ target }) => {
        ...
        if (!isAutoCompleteOpen && input_value !== '' && input_value.length > 0) startOpenAutoComplete()

        if (input_value.length >= 1) { getSearchTags();}
    };
                            ...
                            <ol className=' w-full h-auto py-2 grid divide-y divide-dashed divide-slate-300 px-3'>
                                {searchTags.map(tag => (
                                    <a
                                        ...
                                        key={tag} className=' py-1 cursor-pointer text-sm sm:text-base'
                                    >{tag}</a>
                                    ))}
                             </ol>
                        </div>
