...
import { CategoriesComponent, SubcategoriesComponent } from "/src/pages/components";

export const CategoriesPage = () => {
  const navigate = useNavigate();
  const { categories, startSettingSubCategory, subcategorySelected } =
    useGetCategories();
  return (
    <>
        ...
        <div className="hidden md:block pt-2 md:pt-5 w-4/12 h-48">
          ...
          <ol className="text-white text-md  ">
            {categories.map((category) => (
              <a
                className="grid cursor-pointer py-1 text-white hover:text-cyan-500"
                key={category.name}
                onClick={() => {
