...
import { useGetItemById, useGetResposeBySearchTerm } from "/src/hooks";
import {
  GifSelectedComponent,
  ShowItemButtons,
  UserMakerGifComponent,
        ...
        <div className="bg-transparent p-2  md:flex  mb-5 w-fit  truncate">
          {itemTags.map(({ name }) => (
            <div className="p-1  justify-items-center" key={name}>
              <button
                className=" px-4 w-min-fit h-9 text-sm rounded-full italic font-base
        ...
        {itemGifsRelated.length > 0 ? (
          <div>
            <div className=" w-full grid justify-items-start mx-2">
              <p className=" text-white text-sm md:text-lg font-mono font-semibold w-full ">
                Related Gifs
              </p>
            </div>
